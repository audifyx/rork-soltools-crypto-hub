...
import { ArrowLeft, AtSign, Bell, CheckCheck, Heart, MessageCircle, Repeat2, Rocket, Sparkles, TrendingUp, UserPlus, Waves, Zap } from "lucide-react-native";
...
const KIND_META: Record<NotifKind, { Icon: LucideIcon; color: string; bg: string }> = {
  ...
  moderation_update: { Icon: Sparkles, color: Colors.orange, bg: "rgba(255,184,76,0.16)" },
...
export default function NotificationsScreen() {
  ...
  const items = useMemo(() => {
    ...
    if (merged.length === 0 && isAuthenticated) {
      return [{ id: "welcome", kind: "system", title: "Welcome to Social Alpha Cockpit", body: "Likes, mentions, alerts, and whale moves will land here.", ts: Date.now(), unread: false }] as Notif[];
    }
    return merged;
  ...
  const onTap = useCallback((n: Notif) => {
    tap();
    markLocalRead(n);
    ...
    else if (n.kind === "whale" || n.kind === "trade" || n.kind === "alert") router.push("/(tabs)/discover");
    ...
    <View style={styles.root} testID="notifications-screen">
      ...
      <SafeAreaView edges={["top"]} style={styles.safe}>
        <View style={styles.header}>
          <Pressable onPress={() => navigateBack(router, "/(tabs)/home")} style={styles.iconBtn} testID="notif-back"><ArrowLeft color={Colors.text} size={18} strokeWidth={2.4} /></Pressable>
          <View style={styles.headerCenter}>
            <Text style={styles.title}>Notifications</Text>
            {unreadCount > 0 ? <View style={styles.unreadPill}><View style={styles.unreadDot} /><Text style={styles.unreadText}>{unreadCount} new</Text></View> : null}
          </View>
          <Pressable onPress={onMarkAllRead} style={styles.iconBtn} disabled={unreadCount === 0} testID="mark-all-read"><CheckCheck color={unreadCount > 0 ? Colors.mint : Colors.muted} size={18} strokeWidth={2.4} /></Pressable>
        </View>

        <View style={styles.tabsWrap}>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.tabsRow}>
            {TABS.map((t) => <Pressable key={t.id} onPress={() => { tap(); setTab(t.id); }} style={[styles.tab, tab === t.id && styles.tabActive]} testID={`tab-${t.id}`}><Text style={[styles.tabText, tab === t.id && styles.tabTextActive]}>{t.label}</Text></Pressable>)}
          </ScrollView>
        </View>

        <FlatList
          data={filtered}
          keyExtractor={(i) => i.remoteId ?? i.id}
          renderItem={({ item }) => <NotifRow item={item} unread={item.unread && !(readIds.has(item.id) || (item.remoteId ? readIds.has(item.remoteId) : false))} onPress={() => onTap(item)} />}
          contentContainerStyle={styles.listContent}
          ItemSeparatorComponent={() => <View style={styles.separator} />}
          ListEmptyComponent={remoteQ.isLoading ? <ActivityIndicator color={Colors.mint} style={{ marginTop: 80 }} /> : <EmptyState tab={tab} />}
          showsVerticalScrollIndicator={false}
          onEndReached={() => { if (remoteQ.hasNextPage && !remoteQ.isFetchingNextPage) remoteQ.fetchNextPage().catch(() => {}); }}
...
function NotifRow({ item, unread, onPress }: { item: Notif; unread: boolean; onPress: () => void }) {
  ...
  return <Pressable onPress={onPress} style={[styles.row, unread && styles.rowUnread]} testID={`notif-${item.id}`}>{unread ? <View style={[styles.unreadStripe, { backgroundColor: meta.color }]} /> : null}<View style={[styles.iconWrap, { backgroundColor: meta.bg }]}><meta.Icon color={meta.color} size={16} strokeWidth={2.4} /></View><View style={styles.body}><View style={styles.bodyTop}><Text style={styles.rowTitle} numberOfLines={1}>{item.title}</Text><Text style={styles.rowTime}>{timeAgo(item.ts)}</Text></View><Text style={styles.rowBody} numberOfLines={2}>{item.body}</Text></View></Pressable>;
}

function EmptyState({ tab }: { tab: Tab }) {
  const body = tab === "whales" ? "Whale moves on tracked tokens will land here." : tab === "trades" ? "Price alerts and trade events will land here." : tab === "social" ? "Likes, reposts, and new followers will appear here." : tab === "mentions" ? "Mentions and replies will appear here." : "Likes, mentions, alerts, and whale moves all in one place.";
  return <View style={styles.empty} testID="notif-empty"><Bell color={Colors.mint} size={28} strokeWidth={2.2} /><Text style={styles.emptyTitle}>You're all caught up</Text><Text style={styles.emptyBody}>{body}</Text></View>;
}

const styles = StyleSheet.create({
  root: { flex: 1, backgroundColor: Colors.ink, overflow: "hidden" }, safe: { flex: 1 },
  header: { paddingHorizontal: 14, paddingTop: 4, paddingBottom: 12, flexDirection: "row", alignItems: "center", justifyContent: "space-between" },
  headerCenter: { alignItems: "center", gap: 4 }, title: { color: Colors.text, fontSize: 17, fontWeight: "900", letterSpacing: -0.3 },
  unreadPill: { flexDirection: "row", alignItems: "center", gap: 5, paddingHorizontal: 8, paddingVertical: 3, borderRadius: 999, backgroundColor: "rgba(85,245,178,0.12)", borderWidth: 1, borderColor: "rgba(85,245,178,0.32)" },
  unreadDot: { width: 5, height: 5, borderRadius: 3, backgroundColor: Colors.mint }, unreadText: { color: Colors.mint, fontSize: 10, fontWeight: "900", letterSpacing: 0.4 },
  iconBtn: { width: 36, height: 36, borderRadius: 12, backgroundColor: Colors.card, borderWidth: 1, borderColor: "rgba(255,255,255,0.06)", alignItems: "center", justifyContent: "center" },
  tabsWrap: { borderBottomWidth: 1, borderBottomColor: "rgba(255,255,255,0.06)", marginBottom: 4 }, tabsRow: { paddingHorizontal: 12, gap: 6, paddingBottom: 10 },
  tab: { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 999, backgroundColor: Colors.card, borderWidth: 1, borderColor: "rgba(255,255,255,0.06)" }, tabActive: { backgroundColor: Colors.mint, borderColor: Colors.mint }, tabText: { color: Colors.muted, fontSize: 12, fontWeight: "800" }, tabTextActive: { color: Colors.ink, fontWeight: "900" },
  listContent: { paddingVertical: 6, paddingBottom: 80, flexGrow: 1 }, separator: { height: 1, backgroundColor: "rgba(255,255,255,0.04)", marginLeft: 64 },
  row: { flexDirection: "row", alignItems: "flex-start", gap: 12, paddingHorizontal: 16, paddingVertical: 14, position: "relative" }, rowUnread: { backgroundColor: "rgba(85,245,178,0.04)" }, unreadStripe: { position: "absolute", left: 0, top: 14, bottom: 14, width: 3, borderRadius: 2 },
  iconWrap: { width: 36, height: 36, borderRadius: 12, alignItems: "center", justifyContent: "center" }, body: { flex: 1, gap: 4 }, bodyTop: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 8 }, rowTitle: { color: Colors.text, fontSize: 14, fontWeight: "800", flex: 1, letterSpacing: -0.2 }, rowTime: { color: Colors.muted, fontSize: 11, fontWeight: "700" }, rowBody: { color: Colors.muted, fontSize: 12, fontWeight: "600", lineHeight: 17 },
  empty: { paddingTop: 80, alignItems: "center", paddingHorizontal: 32, gap: 10 }, emptyTitle: { color: Colors.text, fontSize: 17, fontWeight: "900", marginTop: 8, letterSpacing: -0.3 }, emptyBody: { color: Colors.muted, fontSize: 13, fontWeight: "600", textAlign: "center", lineHeight: 19 },
});
