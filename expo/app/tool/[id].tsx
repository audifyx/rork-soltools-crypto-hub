...
function moduleToMeta(module: SolToolsModuleSpec): ToolMeta {
  return {
    ...
    description: module.gatedReason ?? `${module.name} is connected through the existing Crypto Community App API/data layer without changing API contracts.`,
  };
}
...
function ConnectedModuleTool({ meta }: { meta: ToolMeta }) {
  const checks = [
    "Route is registered in the Crypto Community App platform map",
...
function ComingSoonTool({ accent }: { accent: string }) {
  return (
    <EmptyState
      ...
      Icon={Sparkles}
      title="Coming soon"
      body="This module is listed in the Crypto Community App platform map and can be promoted to a live input tool without changing API contracts."
    />
  );
}
