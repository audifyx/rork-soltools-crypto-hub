...
--
-- Adds:
--   • public.post_likes (with RLS + auto-maintained likes_count)
--   • public.whale_events (Whales feed source)
--   • public.get_following_feed(max_rows) RPC for the Following tab
-- =====================================================================
...
do $$
begin
  begin execute 'drop policy if exists "post_likes_read" on public.post_likes'; exception when others then null; end;
  execute $p$create policy "post_likes_read" on public.post_likes for select using (true)$p$;

  begin execute 'drop policy if exists "post_likes_write_self" on public.post_likes'; exception when others then null; end;
  execute $p$create policy "post_likes_write_self" on public.post_likes for insert with check (auth.uid() = user_id)$p$;

  begin execute 'drop policy if exists "post_likes_delete_self" on public.post_likes'; exception when others then null; end;
...
do $$
begin
  begin execute 'drop policy if exists "whale_events_read" on public.whale_events'; exception when others then null; end;
  execute $p$create policy "whale_events_read" on public.whale_events for select using (true)$p$;
end $$;
...
returns table (
  ...
  change_pct numeric, likes_count integer, reposts_count integer,
  comments_count integer, created_at timestamptz
