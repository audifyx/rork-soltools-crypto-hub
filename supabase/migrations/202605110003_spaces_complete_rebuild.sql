-- Complete rebuild of the Spaces / LiveKit voice rooms system.
-- Fully idempotent. Re-creates every table, RPC, RLS policy needed for the
-- Spaces feature to create, host, join, chat, react and end rooms reliably.
-- Safe to run repeatedly.

create extension if not exists pgcrypto;

-- ----------------------------------------------------------------------------
-- TABLES (create if missing) and ensure required columns/defaults exist.
-- ----------------------------------------------------------------------------
...
-- Make sure host_id column exists and is nullable so the fallback insert works.
alter table public.livekit_rooms alter column host_id drop not null;
alter table public.livekit_rooms alter column host_id set default auth.uid();

-- Ensure all optional columns exist for older deployments.
alter table public.livekit_rooms add column if not exists accent_a text default '#F4C65B';
alter table public.livekit_rooms add column if not exists accent_b text default '#55F5B2';
alter table public.livekit_rooms add column if not exists recording boolean not null default false;
alter table public.livekit_rooms add column if not exists raised_hands integer not null default 0;
alter table public.livekit_rooms add column if not exists listeners_count integer not null default 0;
alter table public.livekit_rooms add column if not exists speakers_count integer not null default 0;
alter table public.livekit_rooms add column if not exists category text not null default 'alpha';
alter table public.livekit_rooms add column if not exists scheduled_at timestamptz;
alter table public.livekit_rooms add column if not exists community_id uuid;
...
-- ----------------------------------------------------------------------------
-- RLS — fully reset every policy so direct fallback inserts and RPC reads work
-- regardless of the historical migration order.
-- ----------------------------------------------------------------------------
...
begin
  for pol in select tablename, policyname from pg_policies
             where schemaname = 'public'
               and tablename in ('livekit_rooms','livekit_participants','space_messages','space_reactions','space_follows')
  loop
    execute format('drop policy if exists %I on public.%I', pol.policyname, pol.tablename);
  end loop;
end $$;

-- Read access is public so feeds, rails and detail pages work for guests too.
create policy livekit_rooms_select_all on public.livekit_rooms for select using (true);
create policy livekit_rooms_insert_own on public.livekit_rooms for insert with check (host_id = auth.uid() or auth.uid() is not null);
...
create policy livekit_rooms_delete_host on public.livekit_rooms for delete using (host_id = auth.uid());

create policy livekit_participants_select_all on public.livekit_participants for select using (true);
create policy livekit_participants_insert_self on public.livekit_participants for insert with check (
  user_id = auth.uid() or exists (select 1 from public.livekit_rooms r where r.id = room_id and r.host_id = auth.uid())
);
create policy livekit_participants_update_self_or_host on public.livekit_participants for update using (
  user_id = auth.uid() or exists (select 1 from public.livekit_rooms r where r.id = room_id and r.host_id = auth.uid())
) with check (
  user_id = auth.uid() or exists (select 1 from public.livekit_rooms r where r.id = room_id and r.host_id = auth.uid())
);
create policy livekit_participants_delete_self_or_host on public.livekit_participants for delete using (
  user_id = auth.uid() or exists (select 1 from public.livekit_rooms r where r.id = room_id and r.host_id = auth.uid())
);

create policy space_messages_select_all on public.space_messages for select using (true);
create policy space_messages_insert_self on public.space_messages for insert with check (user_id = auth.uid());
create policy space_messages_delete_self_or_host on public.space_messages for delete using (
  user_id = auth.uid() or exists (select 1 from public.livekit_rooms r where r.id = room_id and r.host_id = auth.uid())
);

create policy space_reactions_select_all on public.space_reactions for select using (true);
create policy space_reactions_insert_self on public.space_reactions for insert with check (user_id = auth.uid());

create policy space_follows_select_own on public.space_follows for select using (user_id = auth.uid());
create policy space_follows_write_own on public.space_follows for all using (user_id = auth.uid()) with check (user_id = auth.uid());

-- ----------------------------------------------------------------------------
-- RPCs (security definer) — drop and re-create so PostgREST cache is refreshed.
-- ----------------------------------------------------------------------------
...
begin
  ...
  insert into public.livekit_rooms(
    ...
    status, is_active, started_at, scheduled_at, category, recording, accent_a, accent_b
  )
  values (
    created_id,
    trimmed_name,
...
drop function if exists public.start_space(uuid);
create function public.start_space(target_room_id uuid)
returns void
language plpgsql
security definer
set search_path = public
as $$
begin
  if auth.uid() is null then raise exception 'Sign in to start this Space.' using errcode = '28000'; end if;
  ...
  if not found then raise exception 'Only the host can start this Space.' using errcode = '42501'; end if;
end;
$$;
...
create function public.leave_space(target_room_id uuid)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  set left_at = now(), speaking = false, hand_raised = false, last_seen_at = now()
  where room_id = target_room_id and user_id = auth.uid();
  perform public.refresh_space_counts(target_room_id);
end;
$$;

drop function if exists public.set_space_mute(uuid, boolean);
create function public.set_space_mute(target_room_id uuid, p_muted boolean)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  if not found then raise exception 'Raise your hand and wait for speaker access.' using errcode = '42501'; end if;
end;
$$;

drop function if exists public.set_space_hand(uuid, boolean);
create function public.set_space_hand(target_room_id uuid, p_raised boolean)
returns void
language plpgsql
security definer
set search_path = public
...
drop function if exists public.set_space_participant_role(uuid, uuid, text);
create function public.set_space_participant_role(target_room_id uuid, target_participant_id uuid, p_role text)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  if not exists (select 1 from public.livekit_rooms where id = target_room_id and host_id = auth.uid()) then
    raise exception 'Only hosts can manage the stage.' using errcode = '42501';
  end if;
  update public.livekit_participants
...
drop function if exists public.remove_space_participant(uuid, uuid);
create function public.remove_space_participant(target_room_id uuid, target_participant_id uuid)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  if not exists (select 1 from public.livekit_rooms where id = target_room_id and host_id = auth.uid()) then
    raise exception 'Only hosts can remove listeners.' using errcode = '42501';
  end if;
  update public.livekit_participants
...
drop function if exists public.send_space_message(uuid, text);
create function public.send_space_message(target_room_id uuid, p_body text)
returns uuid
language plpgsql
security definer
set search_path = public
...
begin
  if auth.uid() is null then raise exception 'Sign in to chat in Spaces.' using errcode = '28000'; end if;
  ...
  values (
    target_room_id, auth.uid(),
    left(trim(p_body), 1000),
    case when trim(p_body) ~ '^[$#][A-Za-z0-9_]+' then 'ticker' else 'text' end
  )
...
drop function if exists public.add_space_reaction(uuid, text);
create function public.add_space_reaction(target_room_id uuid, p_emoji text default '🔥')
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  values (target_room_id, auth.uid(), left(coalesce(p_emoji, '🔥'), 8));
  insert into public.space_messages(room_id, user_id, body, message_type)
  values (target_room_id, auth.uid(), left(coalesce(p_emoji, '🔥'), 8), 'reaction');
end;
$$;

drop function if exists public.end_space(uuid);
create function public.end_space(target_room_id uuid)
returns void
language plpgsql
security definer
set search_path = public
as $$
begin
  if auth.uid() is null then raise exception 'Sign in to end this Space.' using errcode = '28000'; end if;
  update public.livekit_rooms
  set status = 'ended', is_active = false, ended_at = now(), updated_at = now()
  where id = target_room_id and host_id = auth.uid();
  if not found then raise exception 'Only the host can end this Space.' using errcode = '42501'; end if;
  update public.livekit_participants set left_at = coalesce(left_at, now()), speaking = false where room_id = target_room_id;
  perform public.refresh_space_counts(target_room_id);
end;
$$;

drop function if exists public.follow_space(uuid, boolean);
create function public.follow_space(target_room_id uuid, p_follow boolean default true)
returns void
language plpgsql
security definer
set search_path = public
...
-- ----------------------------------------------------------------------------
-- Grants — make sure PostgREST roles can call every RPC.
-- ----------------------------------------------------------------------------
grant execute on function public.create_space(text, text, text, text, timestamptz, boolean, text, text) to anon, authenticated;
grant execute on function public.start_space(uuid) to anon, authenticated;
grant execute on function public.join_space(uuid) to anon, authenticated;
grant execute on function public.leave_space(uuid) to anon, authenticated;
grant execute on function public.set_space_mute(uuid, boolean) to anon, authenticated;
grant execute on function public.set_space_hand(uuid, boolean) to anon, authenticated;
grant execute on function public.set_space_participant_role(uuid, uuid, text) to anon, authenticated;
grant execute on function public.remove_space_participant(uuid, uuid) to anon, authenticated;
grant execute on function public.heartbeat_space_participant(uuid) to anon, authenticated;
grant execute on function public.send_space_message(uuid, text) to anon, authenticated;
grant execute on function public.add_space_reaction(uuid, text) to anon, authenticated;
grant execute on function public.end_space(uuid) to anon, authenticated;
grant execute on function public.follow_space(uuid, boolean) to anon, authenticated;
grant execute on function public.refresh_space_counts(uuid) to anon, authenticated;
